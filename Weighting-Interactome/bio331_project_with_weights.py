...
def main(): # EEK, KT added comments to this
    ...
    interactome = "flybase-weighted-interactome.txt" #weighted interactome file
...
#Input: Three-column text file containing weighted edges in the interactome
#Output: Set of edges and set of nodes in the whole interactome
def read_edge_file(filename): ##taken from L.T.'s code 
    nodes = set()
    edges = set()
    with open (filename, 'r') as f:
        ...
        for line in f:
            k = line.strip().split("\t")
            k[2] = float(k[2]) #converts weight from string to float
            edges.add(tuple(k))
            nodes.add(k[0])
            nodes.add(k[1])
    return edges,nodes
