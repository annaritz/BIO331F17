...
def main(): # EEK, KT added comments to this
    ...
    dbs,evs,pmids = process_edges(edges)
    plot_evs_pmids(evs,pmids)
...
def plot_evs_pmids(evs_full,pmids_full):
    plt.figure(figsize=(6,3))
    ax = plt.subplot(1,2,1)
    thres = 10
    evs = {e:evs_full[e]for e in evs_full  if evs_full[e]>=thres }
    pmids = {p:pmids_full[p] for p in pmids_full if pmids_full[p]>=thres}

    ax.bar(range(len(evs)),sorted(evs.values(),reverse=True),log=True)
    ax.set_title('PSI-MI Evidence Sources')
    ax.set_ylabel('Log # Edges')
    ax.set_xlabel('Evidence Source\n(n=%d with %d or more edges)' % (len(evs),thres))

    ax = plt.subplot(1,2,2)
    ax.bar(range(len(pmids)),sorted(pmids.values(),reverse=True),log=True)
    ax.set_title('PubMed-Indexed Citations')
    ax.set_ylabel('Log # Edges')
    ax.set_xlabel('Citation\n(n=%d with %d or more edges)' % (len(pmids),thres))

    plt.tight_layout()
    plt.savefig('evidence_histogram.png')
    print('saved file to evidence_histogram.png')
    return
    ...
    for row in edges:
        ...
        for e in ev_set:
            if e not in evs:
                ...
                evs[e] += 1   
    print('%d DBs:' % (len(dbs)))
    for db in sorted(dbs,key=lambda x:dbs[x],reverse=True):
        print('  %d\t%s' % (dbs[db],db))
    thres = 10
    print('top %d EVs (out of %d):' % (thres,len(evs)))
    i = 1
    for ev in sorted(evs,key=lambda x:evs[x],reverse=True):
        print('  %d\t%s' % (evs[ev],ev))
        i+=1
        if i > thres:
            break
    print('%d PMIDs:' % (len(pmids)))
    i = 1
    for ev in sorted(pmids,key=lambda x:pmids[x],reverse=True):
        print('  %d\t%s' % (pmids[ev],ev))
        i+=1
        if i > thres:
            break

    return dbs,evs,pmids
